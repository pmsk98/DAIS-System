...
class ReinforcementLearner:
    ...
    def init_value_network(self, shared_network=None, activation='relu', loss='mse'):
        if self.net == 'dnn':
    ...
    def visualize(self, epoch_str, num_epoches, epsilon):
        ...
        self.memory_action = [Agent.ACTION_SELL]*(self.num_steps - 1) + self.memory_action
    ...
    def run(self, learning=True):
        ...
        for epoch in tqdm(range(self.num_epoches)):
            ...
            for i in tqdm(range(len(self.training_data)), leave=False):
                ...
                ###### 수정부분
                #학습기간동안  trading signal 기록   
                action,confidence,_ = self.agent.decide_action(pred_value,pred_policy,0)
                train_result.append((self.environment.observation[0],int(action),float(confidence)))
            ...
            with open(os.path.join(self.output_path,f'predict_train_{self.stock_code}.json'),'w') as f:
                print(json.dumps(train_result),file=f)
